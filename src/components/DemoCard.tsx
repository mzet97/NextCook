...
function DemoCard({ title, description, icon, href, color, children }: DemoCardProps & { children?: React.ReactNode }) {
  ...
  const cardContent = (
    <div className={`card border-2 transition-all duration-300 hover:scale-105 hover:shadow-xl ${borderClasses[color as keyof typeof borderClasses] || borderClasses.blue}`}>
      ...
      {/* Arrow indicator - only show if href is provided */}
      {href && (
  ...
  // Return with Link wrapper if href is provided, otherwise just the card content
  return href ? (
